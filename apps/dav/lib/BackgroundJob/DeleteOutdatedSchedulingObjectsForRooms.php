...
class DeleteOutdatedSchedulingObjectsForRooms extends TimedJob {
	public function __construct(
		private IRoomBackend $roomsBackend,
	...
	protected function run($argument) {
		try {
			$rooms = $this->roomsBackend->listAllRooms();
		...
		foreach ($rooms as $room) {
			$principal = 'principals/calendar-rooms/calendar_resource_management-' . $room;
		...
		$this->logger->info("Removed $count outdated scheduling objects for rooms");
	}
}
